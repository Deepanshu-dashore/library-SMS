...
export default function SeatCalendar() {
  ...
  return (
    <div className={clsx(
      "rounded-xl overflow-hidden flex flex-col min-h-auto font-sans border",
      mode === "dark"
        ? "bg-[#1c252e] border-gray-800 shadow-[0_20px_50px_rgba(0,0,0,0.25)]"
        : "bg-white border-slate-100 shadow-[0_20px_50px_rgba(0,0,0,0.05)]"
    )}>

      {/* Header Controls */}
      <div className={clsx(
        "px-8 py-5 flex flex-col md:flex-row md:items-center justify-between gap-6 border-b",
        mode === "dark" ? "bg-[#1a212a] border-gray-800" : "bg-slate-50/50 border-slate-100"
      )}>
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className={clsx("text-xl font-public-sans font-bold leading-none group cursor-default", mode === "dark" ? "text-white" : "text-gray-900")}>
              {currentDate.toLocaleString('default', { month: 'long' })}
              <span style={{ color }} className="ml-2 font-barlow">{year}</span>
            </h2>
            <p className={clsx("text-[12px] font-public-sans mt-0.5", mode === "dark" ? "text-gray-400" : "text-gray-500")}>Seat Availability Timeline</p>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={clsx(
              "px-4 py-2 text-xs font-public-sans font-semibold gap-2 flex items-center justify-center border rounded-lg cursor-pointer transition-all active:scale-95",
              mode === "dark"
                ? "bg-slate-900 border-gray-800 text-slate-400 hover:text-white"
                : "bg-white border-slate-200 text-slate-400 hover:text-[var(--hover-color)] hover:border-[var(--hover-color)]/20 hover:shadow-lg hover:shadow-[var(--hover-color)]/5"
            )}
          >
      ...
      <div className="flex-1 overflow-hidden relative flex flex-col">
        <div ref={scrollContainerRef} className="overflow-x-auto overflow-y-auto max-h-[calc(100vh-280px)] custom-scrollbar">
          <div className="inline-block min-w-full">
            ...
            {/* Days Header */}
            <div className={clsx("flex sticky top-0 z-40 border-b", mode === "dark" ? "bg-[#141b22] border-gray-800" : "bg-white border-slate-200")}>
              <div className={clsx(
                "w-42 shrink-0 p-4 border-r backdrop-blur-sm flex items-center gap-2",
                mode === "dark" ? "border-gray-800 bg-slate-900/80" : "border-slate-200 bg-slate-50/80"
              )}>
                <Layers size={13} className="text-slate-400" />
                <span className={clsx("text-[10px] font-bold uppercase tracking-widest", mode === "dark" ? "text-slate-400" : "text-slate-500")}>Library Assets</span>
              </div>
              <div className="flex">
                {daysArray.map(day => {
                  ...
                  return (
                    <div
                      key={day}
                      className={clsx(
                        "w-12 h-14 flex flex-col items-center justify-center shrink-0 transition-colors border-r",
                        mode === "dark" ? "border-gray-800/80" : "border-slate-200/80",
                        isToday
                          ? ""
                          : isGrayColumn
                            ? (mode === "dark" ? "bg-slate-900/60" : "bg-slate-100/70")
                            : (mode === "dark" ? "bg-[#1c252e]" : "bg-white"),
                        isWeekend && !isToday ? (mode === "dark" ? "bg-slate-900/30" : "bg-slate-100/40") : ""
                      )}
                      style={isToday ? { backgroundColor: color } : {}}
                    >
                      <span className={clsx(
                        "text-[8px] font-public-sans font-bold uppercase tracking-tighter mb-0.5",
                        isToday ? "text-white" : (mode === "dark" ? "text-slate-500" : "text-slate-400")
                      )}>
                        {date.toLocaleDateString('default', { weekday: 'short' })}
                      </span>
                      <span className={clsx(
                        "text-[13px] font-public-sans font-bold leading-none",
                        isToday ? "text-white" : (mode === "dark" ? "text-slate-300" : "text-slate-800")
                      )}>
                        {day}
                      </span>
                    </div>
            ...
            <div className="relative">
              {loading ? (
                <div className="p-20 text-center flex flex-col items-center gap-4 animate-pulse">
                  <div className={clsx("w-10 h-10 border-4 rounded-full animate-spin", mode === "dark" ? "border-slate-800" : "border-slate-100")} style={{ borderTopColor: color }} />
                  <span className={clsx("text-[10px] font-public-sans font-bold uppercase tracking-widest", mode === "dark" ? "text-slate-500" : "text-slate-400")}>Loading Grid...</span>
                </div>
              ) : Object.entries(groupedSeats).map(([floor, floorSeats], fIdx) => (
                <div key={floor}>
                  {/* Floor Label */}
                  <div className={clsx("flex border-b", mode === "dark" ? "bg-slate-900/40 border-gray-800/50" : "bg-slate-100/60 border-slate-200/50")}>
                    <div className="w-full py-1.5 px-5 sticky left-0 z-20">
                      <span className={clsx("text-[10px] font-public-sans font-bold uppercase tracking-[0.2em]", mode === "dark" ? "text-gray-400" : "text-slate-400")}>{floor}</span>
                    </div>
                  </div>
                  ...
                  {floorSeats.map(seat => {
                    ...
                    return (
                      <div key={seat._id} className={clsx("flex border-b h-11 group", mode === "dark" ? "border-gray-800/60" : "border-slate-200/60")}>
                        {/* Seat Name sticky cell */}
                        <div className={clsx(
                          "w-42 shrink-0 px-5 flex items-center gap-2 border-r sticky left-0 transition-colors z-20",
                          mode === "dark"
                            ? "border-gray-800 bg-[#1c252e] group-hover:bg-[#151b22]"
                            : "border-slate-200 bg-white group-hover:bg-slate-50"
                        )}>
                          <div className="w-1.5 h-1.5 rounded-full bg-emerald-400 shrink-0" />
                          <span className={clsx("text-[11px] font-public-sans font-bold", mode === "dark" ? "text-gray-300" : "text-slate-600")}>Seat {seat.seatNumber}</span>
                          <StatusBadge
                            status={seat.type}
                            size="xxs"
                            className={clsx(
                              ...
                              seat.type.toLowerCase() === 'ac'
                                ? (mode === 'dark' ? 'bg-indigo-950/40 text-indigo-400 border-indigo-900/50' : 'bg-indigo-100/70 text-indigo-600 border-indigo-100')
                                : (mode === 'dark' ? 'bg-slate-900/60 text-slate-400 border-slate-800' : 'bg-slate-200/60 text-slate-500 border-slate-100')
                            )}
                          />
                        </div>
                          ...
                          {daysArray.map(day => {
                            ...
                            return (
                              <div
                                key={day}
                                className={clsx(
                                  "w-12 h-full shrink-0 border-r",
                                  mode === "dark" ? "border-gray-800/60" : "border-slate-200/60",
                                  isGrayColumn ? (mode === "dark" ? "bg-slate-950/20" : "bg-slate-100/50") : "",
                                  isWeekend ? (mode === "dark" ? "bg-slate-950/40" : "bg-slate-100/20") : ""
                                )}
                              />
                            )
                          })}
        ...
        <div className={clsx(
          ...
          mode === "dark" ? "border-gray-800 bg-[#1c252e]" : "border-slate-100 bg-white"
        )}>
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
              <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: color }} />
              <span className={clsx("text-[9px] font-public-sans font-bold uppercase", mode === "dark" ? "text-gray-400" : "text-slate-400")}>Booked Seats</span>
            </div>
            <div className="flex items-center gap-2">
              <div className={clsx("w-2.5 h-2.5 rounded-full", mode === "dark" ? "bg-slate-900 border border-gray-800" : "bg-slate-100")} />
              <span className={clsx("text-[9px] font-public-sans font-bold uppercase", mode === "dark" ? "text-gray-400" : "text-slate-400")}>Available</span>
            </div>
          </div>
          <div className={clsx("flex items-center gap-2", mode === "dark" ? "text-gray-600" : "text-slate-300")}>
            <Info size={12} />
            <span className={clsx("text-[9px] font-public-sans font-bold italic", mode === "dark" ? "text-gray-500" : "text-slate-400/80")}>Gantt grid showing daily duration for each member.</span>
          </div>
        </div>
      </div>
    </div>
