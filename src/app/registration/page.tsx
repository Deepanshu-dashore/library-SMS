...
const StepItem = ({
  ...
  >
    <div className="step-number">
      {completed ? <Icon icon="solar:check-read-bold" /> : id}
    </div>
    ...
    <div className="input-wrapper">
      {type === "select" ? (
        <>
          <select
            {...props}
            className={`custom-input appearance-none ${props.disabled ? "bg-gray-50 cursor-not-allowed opacity-75" : ""}`}
          >
        ...
        <textarea
          {...props}
          className={`custom-input min-h-[100px] resize-none ${props.disabled ? "bg-gray-50 cursor-not-allowed opacity-75" : ""}`}
        />
      ) : type === "date" ? (
        <>
          <input
            {...props}
            type="date"
            className={`custom-input ${props.disabled ? "bg-gray-50 cursor-not-allowed opacity-75" : ""}`}
          />
...
const UploadZone = ({
  ...
  <div className="flex flex-col gap-4">
    ...
    {preview ? (
      <div
        className={`preview-box ${field === "signature" ? "signature" : ""} ${disabled ? "opacity-90 grayscale-[0.2]" : ""}`}
      >
...
export default function RegisterPage() {
  ...
  const onCropComplete = useCallback(
    (_initialed: any, croppedAreaPixels: any) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const loadingToast = toast.loading(
      "Processing your membership and uploading documents...",
    );
    ...
    try {
        ...
        if (key === "address") {
          Object.keys(formData.address).forEach((addrKey) => {
            formDataToSend.append(
              `address.${addrKey}`,
              formData.address[addrKey as keyof typeof formData.address],
            );
          });
      ...
      if (data.success) {
        toast.success(
          `Welcome to the library, ${formData.name}! Registration successful.`,
  ...
  const removeFile = (field: "photo" | "signature") => {
    ...
    if (field === "signature" && signatureInputRef.current)
          ...
          >
            ...
            <div className="flex flex-col gap-2 mb-8 items-center">
              ...
              <p className="text-slate-400 font-medium text-sm leading-relaxed max-w-xs transition-all">
                Member profile for{" "}
                <span className="text-slate-900 font-bold underline decoration-[#98c156] decoration-2 underline-offset-4">
                  {formData.name}
                </span>{" "}
            ...
            <div className="bg-slate-50/50 rounded-2xl p-4 md:p-6 mb-8 md:p-10 border border-slate-100 flex flex-col items-center">
              ...
              <p className="text-xl md:text-2xl font-black text-slate-800 tracking-wider">
                #{registeredId.slice(-8).toUpperCase()}
              </p>
            </div>
              ...
              <button
                onClick={() => {
                  navigator.clipboard.writeText(
                    `${window.location.origin}/status/${registeredId}`,
                  );
                  toast.success("Tracking link copied!");
                }}
                className="flex-1 bg-white border border-slate-200 py-4 rounded-2xl text-slate-600 font-bold text-sm flex items-center justify-center gap-2 hover:bg-slate-50 transition-all border-b-4 active:border-b-0 active:translate-y-1"
              >
                <Icon icon="solar:link-bold" /> Copy Link
              </button>
              <Link
                href={`/status/${registeredId}`}
                className="flex-1 bg-indigo-600 py-4 rounded-2xl text-white font-bold text-sm flex items-center justify-center gap-2 hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 border-b-4 border-indigo-800 active:border-b-0 active:translate-y-1"
              >
            ...
            <Link
              href="/"
              className="inline-flex items-center gap-2 mt-12 text-slate-400 font-bold text-[10px] hover:text-[#98c156] transition-colors uppercase tracking-[0.2em]"
            >
  ...
  return (
    <div className="register-container bg-linear-to-bl from-[#dff3f8] via-[var(--card)] to-[rgba(255,241,241,1)] bg-opacity-90 backdrop-blur-md  border-gray-300">
      <header className="register-header">
        <Link href="/" className="flex flex-col sm:flex-row items-center gap-4 overflow-visible">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col font-public-sans">
              ...
              <p
                className="text-[10px] md:text-xs font-barlow font-semibold capitalize leading-tight"
        ...
        <div className="login-link flex items-center gap-4">
          ...
          >
            <Icon
              ...
              className="text-white text-2xl md:text-3xl group-hover:text-gray-700"
            />{" "}
            Check Status
          </button>
        </div>
                ...
                {step === 3 && (
                  ...
                  >
                    <UploadZone
                      ...
                      icon="material-symbols-light:monochrome-photos"
                      required
                    ...
                    <UploadZone
                      field="signature"
                      label="E-Signature"
                      icon="streamline-freehand:cash-payment-pen-signature"
                      required
                      hint="Sign on white paper and upload"
                ...
                {step === 4 && (
                  ...
                  >
                    ...
                    <div className="md:col-span-2 mt-8">
                      <label
                        className={`flex items-start gap-4 p-5 rounded-2x border transition-all cursor-pointer ${declared ? "bg-green-50 border-green-100 shadow-sm" : "bg-slate-50 border-slate-100 hover:bg-slate-100/50"}`}
                      >
                        <input
                          type="checkbox"
                          checked={declared}
          ...
          >
            ...
            >
              ...
              <p className="text-slate-600 text-sm font-semibold leading-relaxed mb-10">
                Enter your unique Application ID to see the current status of
                your registration.
              </p>

              <form onSubmit={handleCheckStatus} className="w-full">
                <div className="mb-10">
                  <label className="text-xs font-semibold capitalize text-slate-700 block mb-3">
                    Your Application ID
                  </label>
                  <input
                    type="text"
                ...
                <button
                  type="submit"
                  className="w-full bg-[#1e293b] text-white py-4 rounded-lg font-medium cursor-pointer text-sm hover:bg-slate-800 transition-all shadow-lg active:scale-[0.98]"
                >
                  Check Status Now
                </button>
              </form>
            ...
            <div className="cropper-controls">
              ...
              <div className="flex gap-4 w-full">
                ...
                <button
                  ...
                  className="flex-1 py-4 bg-indigo-600 rounded-2xl text-white font-bold shadow-lg shadow-indigo-500/30 hover:bg-indigo-700 transition-all"
                >
                  Apply Crop
                </button>
              </div>
