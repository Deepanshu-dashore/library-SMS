...
export function FilterChips({ filters, onClearAll, className = "" }: FilterChipsProps) {
  ...
  return (
    <div className={clsx(
      "col-span-full flex flex-wrap items-center gap-3 pt-4 border-t border-gray-100",
      mode === "dark" && "!border-gray-800",
      className
    )}>
      <span className={clsx("text-[13px] font-semibold text-gray-400 mr-2", mode === "dark" && "!text-slate-500")}>
        Filters applied:
      </span>
      ...
      <AnimatePresence mode="popLayout">
        {filters.map((filter) => (
          <motion.div
            ...
            className={clsx(
              "flex items-center gap-2 border border-gray-200 border-dashed rounded-xl px-2.5 py-1.5 bg-white transition-all",
              mode === "dark" && "!bg-[#1c252e] !border-gray-700"
            )}
          >
            <span className={clsx("text-[13px] font-bold text-gray-800", mode === "dark" && "!text-slate-300")}>
              {filter.label}:
            </span>
            <span className={clsx(
              "flex items-center gap-1.5 bg-gray-100 text-gray-600 text-xs font-semibold px-2 py-0.5 rounded-lg capitalize transition-colors",
              mode === "dark" && "!bg-slate-800 !text-slate-300"
            )}>
              {filter.value}
              <button 
                onClick={filter.onRemove} 
                className={clsx(
                  "cursor-pointer text-white rounded-full p-0.5 transition-colors",
                  mode === "dark" ? "bg-slate-600 hover:bg-slate-500" : "bg-gray-400 hover:bg-gray-500"
                )}
              >
