...
export function FilterChips({ filters, onClearAll, className = "" }: FilterChipsProps) {
    ...
    return (
        <div className={`col-span-full flex flex-wrap items-center gap-3 pt-4 border-t border-gray-100 ${className}`}>
            ...
            <AnimatePresence mode="popLayout">
                {filters.map((filter) => (
                    <motion.div
                        ...
                        className="flex items-center gap-2 border border-gray-200 border-dashed rounded-xl px-2.5 py-1.5 bg-white"
                    >
                        <span className="text-[13px] font-bold text-gray-800">{filter.label}:</span>
                        <span className="flex items-center gap-1.5 bg-gray-100 text-gray-600 text-xs font-semibold px-2 py-0.5 rounded-lg capitalize">
                            ...
                            <button 
                                ...
                                className="bg-gray-400 cursor-pointer hover:bg-gray-500 text-white rounded-full p-0.5 transition-colors"
                            >
